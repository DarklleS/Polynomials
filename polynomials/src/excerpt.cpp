    ...
    std::vector<fp_t>& errors)
{
    ...
    for (int i = 0; i < N_roots_to_check; ++i) {
        ...
        for (int j = 0; j < N_roots_ground_truth; ++j) {
            ...
            //roots_to_check[i] = deviation1 < deviation2 ? roots_to_check[i] : -roots_to_check[i];
        ...
        if (relative_error_for_this_root > 3.99)
        {
            std::cout << relative_error_for_this_root << std::endl <<
                ...
                (std::max(std::abs(roots_ground_truth[j_closest_root]), std::abs(roots_to_check[i_closest_root])) + std::numeric_limits<fp_t>::epsilon()) << std::endl <<
                roots_ground_truth[j_closest_root] << " " << roots_to_check[i_closest_root] << std::endl <<
                (std::max(std::abs(roots_ground_truth[j_closest_root]), std::abs(roots_to_check[i_closest_root])) + std::numeric_limits<fp_t>::epsilon()) - (deviation_min_for_this_root + std::numeric_limits<fp_t>::epsilon()) << std::endl << std::endl;
        }
